...
> 提示：运行 `python install_wizard.py` 可自动生成平台对应的一键启动脚本。也可直接依赖 MCP Server 的**自动检测启动**功能（见下文）。
...
> 运行 `python install_wizard.py` 可自动完成上述配置。
...
| `install_wizard.py` | **一键安装** — 交互式环境检查/安装/配置向导 |
