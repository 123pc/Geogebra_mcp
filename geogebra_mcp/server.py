...
@mcp.tool()
async def geogebra_run_commands(commands: list) -> str:
    """
    Execute multiple GeoGebra commands from a structured list.
    Preferred over geogebra_batch for AI clients — no JSON string wrapping needed.

    Args:
        commands: Ordered GeoGebra commands, e.g. ["A=(0,0)", "B=(6,0)", "Segment(A,B)"]
    """
    try:
...
async def geogebra_create_construction(name: str, design: dict, output_dir: str = "") -> str:
    """
    Create a construction from a structured design dict. Preferred over geogebra_draw_mechanism
    for AI clients — no JSON string wrapping needed.

    design dict format:
    {"perspective":"G", "animate":"α", "speed":0.5,
     "commands":["O1=(0,0)","O2=(6,0)","α=45°",...],
